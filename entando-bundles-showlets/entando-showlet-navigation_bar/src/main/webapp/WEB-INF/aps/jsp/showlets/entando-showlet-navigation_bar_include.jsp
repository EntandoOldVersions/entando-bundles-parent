...
<c:if test="${previousLevel < level}">
	...
	<li<c:out value="${liClass}" escapeXml="false" />><a href="<c:out value="${previousPage.url}" />"<c:out value="${aClassAndData}" escapeXml="false" />><!-- [ <c:out value="${previousLevel}" /> ] --><c:out value="${homeIcon}" escapeXml="false" /><c:out value="${previousPage.title}" /><c:out value="${caret}" escapeXml="false" /></a>
